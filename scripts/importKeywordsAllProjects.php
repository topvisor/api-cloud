...
// импортировать запросы во все проекты
...
$keywords = file_get_contents(__DIR__.'/import.csv');

// импортируем запросы
foreach ($projects as $project) {
	$keywordsImportPen = new TV\Pen($TVSession, 'add', 'keywords_2', 'keywords/import');
	$keywordsImportPen->setData([
		'project_id' => $project->id,
		'keywords' => $keywords,
	]);
	$keywordsImportPenResult = $keywordsImportPen->exec();

	$result = $keywordsImportPenResult->getResult();
	if (is_null($result)) vd($keywordsImportPenResult->getErrors(), 1);
	vd($result);
}
